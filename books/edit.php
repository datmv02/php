...
if ($id == -1) {
    ...
    $getId = "SELECT * FROM books inner join categories on categories.cateid = books.cateid where bookid = $id";
    $resultGetId = exeQuery($getId, false);
    ...
    if (isset($_POST['submit'])) {
        ...
        if ($is_flag == true) {
            $update = "UPDATE books set `title`='$title',`price`=$price,`detail`='$detail',`image`='$image',`cateid`=$cate where bookid = $id";
            exeQuery($update,false);
            header("location:index.php");
        }
    }
}
        ...
        <div class="row" style="justify-content: center;">
            <form action="" method="post" enctype="multipart/form-data" class="col-sm-4">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control <?= isset($vali['title']) ? $vali['title'] : "" ?>" name="title" value="<?= isset($resultGetId['title']) ? $resultGetId['title'] : "" ?>">
                ...
                <div class="form-group">
                    ...
                    <select name="cate" class="form-control <?= isset($vali['cate']) ? $vali['cate'] : "" ?>" value="<?= isset($cate) ? $cate : "" ?>">
                        <option value="<?= $resultGetId['cateid'] ?>"><?= $resultGetId['catename'] ?></option>
                ...
                <div class="form-group">
                    ...
                    <textarea type="text" rows="3" class="form-control <?= isset($vali['detail']) ? $vali['detail'] : "" ?>" name="detail" value=""><?= isset($resultGetId['detail']) ? $resultGetId['detail'] : "" ?></textarea>
                    <div class="invalid-feedback">
                        detail require!!
                    </div>
                </div>
